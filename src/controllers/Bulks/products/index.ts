...
export const ProductsBulkControllers = async (req: Request, res: Response) => {
  try {
    ...
    if (updater) {
      ...
      const bufferToJsonUpdater = excelToJson({
        //@ts-ignore
        source: tmp.data,
        header: {
          rows: 23,
      ...
      const dataParsed = productDataParsed(bufferToJsonUpdater.Worksheet);
      const processWorker = new Worker(
        "./src/controllers/Bulks/products/workers/workerUpdater/worker.import.js",
        {
          workerData: {
            path: "./index.ts",
            params: {
      ...
      return res.send({ ...bufferToJsonUpdater.Worksheet });
    }
