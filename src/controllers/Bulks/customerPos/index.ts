...
import { transformJSON } from '../sellInvoice/functions';

export const CustomerPos = async (req: Request, res: Response) => {
  try {
    const pubSub: PubSub = req.app.get('pubSub');
    const prisma: PrismaClient = req.app.get('prisma');
    const { organizationUuid } = req.body//req.body.payload as Payload;
    ...
    const processWorker = new Worker(
      './src/controllers/Bulks/customerPos/workers/worker.import.js',
      {
        workerData: {
          path: './index.ts',
          params: {
